...
import { Camera, FileText, ImageIcon, Zap, Shield, Globe, Check, ChevronRight, Code2, Star } from "lucide-react"
import { CodeTabs } from "./components/code-tabs"

const PLANS = [
  {
    name: "Free", price: "$0", highlight: false, cta: "Get started free", href: "/login",
    features: ["100 requests/month", "Screenshot API", "PDF API", "OG Image API", "Community support"],
  },
  {
    name: "Starter", price: "$19", highlight: false, cta: "Start Starter", href: "/login?plan=STARTER",
    features: ["1,000 requests/month", "All Free features", "Custom viewport size", "Email support", "90-day log history"],
  },
  {
    name: "Pro", price: "$49", highlight: true, cta: "Start Pro", href: "/login?plan=PRO",
    features: ["10,000 requests/month", "All Starter features", "Retina (2x) screenshots", "Full-page capture", "Priority support", "Webhooks"],
  },
  {
    name: "Business", price: "$149", highlight: false, cta: "Start Business", href: "/login?plan=BUSINESS",
    features: ["100,000 requests/month", "All Pro features", "Custom HTTP headers", "JS injection", "99.9% SLA", "Dedicated support"],
  },
]
...
const FEATURES = [
  ...
  { icon: FileText, title: "HTML to PDF", description: "Turn any webpage into a printable PDF with custom margins, paper formats, and backgrounds." },
  { icon: ImageIcon, title: "OG image generation", description: "Generate beautiful Open Graph social cards on-the-fly with custom titles and themes." },
      ...
      <section className="pt-32 pb-20 px-6 text-center">
        <div className="max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-700 text-xs font-medium px-3 py-1.5 rounded-full mb-6">
            ...
            <span>100 free screenshots every month. No credit card needed.</span>
          </div>
          ...
          <p className="text-xl text-slate-600 max-w-2xl mx-auto mb-10">
            Capture screenshots, generate PDFs, and create OG images via a simple REST API.
            ...
            {[
              ...
              <div key={ep.path} className="border border-slate-200 rounded-xl p-6 hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ep.params.map((p) => (
                    <span key={p} className="bg-slate-100 text-slate-600 text-xs px-2 py-0.5 rounded font-mono">{p}</span>
                  ))}
                </div>
              </div>
            ))}
